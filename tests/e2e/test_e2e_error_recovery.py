...
class TestErrorRecoveryE2E:
    ...
    def test_checkpoint_mechanism(self, mocker, sample_repo_path, e2e_config, disable_api_calls):
        ...
        e2e_config.output_directory = os.path.join(
            tempfile.gettempdir(), "qa_agent_checkpoint_tests"
        )
        ...
        checkpoint_dir = os.path.join(
            tempfile.gettempdir(), "qa_agent_checkpoint_tests", "checkpoints"
        )
    ...
    def test_circuit_breaker_pattern(self, mocker, sample_repo_path, e2e_config, disable_api_calls):
        ...
        e2e_config.repo_path = sample_repo_path
        e2e_config.output_directory = os.path.join(
            tempfile.gettempdir(), "qa_agent_circuit_breaker_tests"
        )
        os.makedirs(e2e_config.output_directory, exist_ok=True)
    ...
    @pytest.mark.e2e
    def test_workflow_with_error_recovery(
        ...
        e2e_config.repo_path = sample_repo_path
        e2e_config.output_directory = os.path.join(
            tempfile.gettempdir(), "qa_agent_error_recovery_tests"
        )
        os.makedirs(e2e_config.output_directory, exist_ok=True)
    ...
    @pytest.mark.e2e
    def test_error_diagnostics_collection(
        self, mocker, sample_repo_path, e2e_config, disable_api_calls
    ):
        ...
        e2e_config.repo_path = sample_repo_path
        e2e_config.output_directory = os.path.join(
            tempfile.gettempdir(), "qa_agent_diagnostics_tests"
        )
        os.makedirs(e2e_config.output_directory, exist_ok=True)
        ...
        value_error_log = next(
            (log for log in error_logs if "ValueError" in log.get("exception_type", "")), None
        )
        assert value_error_log is not None
        assert "test_op_1" in value_error_log.get("operation", "")

        type_error_log = next(
            (log for log in error_logs if "TypeError" in log.get("exception_type", "")), None
        )
        assert type_error_log is not None
        assert "test_op_1" in type_error_log.get("operation", "")

        syntax_error_log = next(
            (log for log in error_logs if "SyntaxError" in log.get("exception_type", "")), None
        )
