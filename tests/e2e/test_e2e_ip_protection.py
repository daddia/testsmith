...
class TestIPProtectionE2E:
    ...
    @pytest.mark.e2e
    def test_ip_protection_basic_redaction(
...
"""
    ...
    @pytest.mark.e2e
    def test_ip_protection_with_code_files(
        self, mocker, sample_repo_path, e2e_config, disable_api_calls
    ):
        ...
        e2e_config.output_directory = os.path.join(
            tempfile.gettempdir(), "qa_agent_ip_protection_tests"
        )
        ...
        # Verify redactions
        assert (
            redacted_regular.content == regular_file.content
        )  # Regular file shouldn't be redacted
        assert (
            redacted_sensitive.content != sensitive_file.content
        )  # Sensitive file should be redacted
        assert "[REDACTED PROTECTED FILE]" in redacted_sensitive.content

    @pytest.mark.e2e
    def test_ip_protection_with_functions(
        self, mocker, sample_repo_path, e2e_config, disable_api_calls
    ):
        ...
        # Verify redactions
        assert (
            redacted_regular.code == regular_function.code
        )  # Regular function shouldn't be redacted
        assert (
            redacted_sensitive1.code != sensitive_function1.code
        )  # Sensitive function should be redacted
        assert "[REDACTED PROTECTED FUNCTION]" in redacted_sensitive1.code
        assert (
            redacted_sensitive2.code != sensitive_function2.code
        )  # Sensitive function should be redacted
        assert "[REDACTED PROTECTED FUNCTION]" in redacted_sensitive2.code

    @pytest.mark.e2e
    def test_ip_protection_with_test_generation(
        self, mocker, sample_repo_path, e2e_config, disable_api_calls
    ):
        ...
        mocker.patch.object(test_generator.test_generator, "save_test_to_file", return_value=None)
    ...
    @pytest.mark.e2e
    def test_load_protection_rules_from_file(
        self, mocker, sample_repo_path, e2e_config, disable_api_calls
    ):
