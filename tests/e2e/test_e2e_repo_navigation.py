...
class TestRepoNavigationE2E:
    ...
    def test_find_related_files(self, mocker, sample_repo_path, e2e_config, disable_api_calls):
        ...
        additional_files = [
            ...
            (
                "sample_module/models.py",
                "from .core.base import Base\n\nclass User(Base):\n    pass",
            ),
            (
                "sample_module/services.py",
                "from .models import User\n\ndef get_user(id):\n    return User()",
            ),
            (
                "tests/test_models.py",
                "from sample_module.models import User\n\ndef test_user():\n    assert User() is not None",
            ),
        ]
        ...
        mocker.patch.object(navigator, "_extract_imports", side_effect=mock_extract_imports)
    ...
    def test_find_function_examples(self, mocker, sample_repo_path, e2e_config, disable_api_calls):
        ...
        # Verify the results
        # Should find usage in calculator.py, api.py, and test_calculator.py
    ...
    def test_should_ignore_patterns(self, mocker, sample_repo_path, e2e_config, disable_api_calls):
        ...
        assert (
            navigator.should_ignore(os.path.join(sample_repo_path, "node_modules/example/index.js"))
            is True
        )
        assert (
            navigator.should_ignore(
                os.path.join(sample_repo_path, "sample_module/__pycache__/cache.py")
            )
            is True
        )
        ...
        assert (
            navigator.should_ignore(os.path.join(sample_repo_path, "sample_module/main.py"))
            is False
        )
        assert (
            navigator.should_ignore(os.path.join(sample_repo_path, "valid_dir/valid.py")) is False
        )
