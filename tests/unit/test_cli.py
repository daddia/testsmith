    ...
    with patch('qa_agent.cli.load_config', return_value=mock_config):
        with patch('qa_agent.cli.logger') as mock_logger:
            with pytest.raises(SystemExit):
                run_cli(mock_args)
                mock_logger.error.assert_called_with("Repository path is required", extra={"error_type": "missing_repo_path"})
...
def test_run_cli_invalid_repo_path(mock_config, mock_args):
    ...
    with patch('qa_agent.cli.load_config', return_value=mock_config):
        with patch('qa_agent.cli.logger') as mock_logger:
            with pytest.raises(SystemExit):
                run_cli(mock_args)
                mock_logger.error.assert_called_with(f"Repository path does not exist: {mock_config.repo_path}", 
                        ...
                        with patch('qa_agent.cli.os.isatty', return_value=False):
                            ...
                            with patch('qa_agent.cli.sys.exit') as mock_exit:
                                with patch('qa_agent.cli.QAWorkflow') as mock_workflow:
                                    ...
                                    mock_instance.run.return_value = {"success_count": 0, "failure_count": 0, "functions": []}
...
def test_run_cli_output_directory_creation(mock_config, mock_args):
    ...
    # Add necessary mocks
    with patch('qa_agent.cli.load_config', return_value=mock_config):
        with patch('qa_agent.cli.update_config_from_args', return_value=mock_config):  # Keep config consistent
            with patch('qa_agent.cli.os.path.exists', return_value=True):  # Mock path exists check
                with patch('qa_agent.cli.os.makedirs') as mock_makedirs:
                    with patch('qa_agent.cli.QAWorkflow') as mock_workflow:
                        ...
                        mock_makedirs.assert_called_with(mock_config.output_directory, exist_ok=True)
...
def test_run_cli_parallel_workflow(mock_config, mock_args):
    ...
    # Add necessary mocks
    with patch('qa_agent.cli.load_config', return_value=mock_config):
        with patch('qa_agent.cli.update_config_from_args', return_value=mock_config):  # Prevent config changes
            with patch('qa_agent.cli.os.path.exists', return_value=True):  # Mock path exists check
                with patch('qa_agent.cli.os.makedirs'):  # Prevent actual directory creation
                    with patch('qa_agent.cli.logger') as mock_logger:
                        with patch('qa_agent.cli.ParallelQAWorkflow') as mock_parallel_workflow:
                            ...
                            assert any(
                                "parallel" in str(call).lower() and 
                                str(mock_config.max_workers) in str(call) 
                ...
                with patch('qa_agent.cli.os.makedirs'):  # Prevent actual directory creation
                    with patch('qa_agent.cli.logger') as mock_logger:
                        with patch('qa_agent.cli.QAWorkflow') as mock_sequential_workflow:
                            ...
                            assert any("sequential" in str(call).lower() for call in mock_logger.info.call_args_list)
                    ...
                    with patch('qa_agent.cli.logger') as mock_logger:
                        with patch('qa_agent.cli.QAWorkflow') as mock_workflow:
                            ...
                            with patch('qa_agent.cli.sys.exit') as mock_exit:
                                ...
                                abort_message_logged = any("abort" in str(call).lower() 
                                                          for call in mock_logger.info.call_args_list)
                        ...
                        with patch('qa_agent.cli.QAWorkflow') as mock_workflow:
                            ...
                            with patch('qa_agent.cli.log_exception') as mock_log_exception:
                                ...
                                with patch('qa_agent.cli.sys.exit') as mock_exit:
                                    ...
                                    assert mock_log_exception.called, "Exception logging function wasn't called"
                                    ...
                                    assert exception_logged, "Our test exception wasn't passed to log_exception"
                                    
                                    # Verify sys.exit was called with error code
                                    assert mock_exit.call_count > 0, "sys.exit wasn't called"
                                    assert any(call(1) == x for x in mock_exit.call_args_list), "sys.exit(1) wasn't called"