...
class TestValidateTest:
    ...
    def _validate_test(self, state, logger, checkpoint_manager, test_validation_agent, console_analysis_agent):
        ...
        if state.get('test_result') and state.get('recovery_attempts', 0) > 0:
            logger.info(f"Resuming with previously validated test for {generated_test.function.name}")
            return state
        ...
        try:
            ...
            # Save error checkpoint
            error_checkpoint = checkpoint_manager.save(state, f"error_validate_{generated_test.function.name}")
            if error_checkpoint:
                state['checkpoints'][f'error_validate_{generated_test.function.name}'] = error_checkpoint
    ...
    def test_validate_test_with_existing_test_result(self, mocker, mock_generated_test, mock_test_result):
        # Arrange
        logger = mocker.MagicMock()
        checkpoint_manager = mocker.MagicMock()
        test_validation_agent = mocker.MagicMock()
        ...
        # Assert
        assert result == state
        logger.info.assert_called_with(f"Resuming with previously validated test for {mock_generated_test.function.name}")

    def test_validate_test_successful_validation(self, mocker, mock_generated_test, mock_test_result):
        # Arrange
        logger = mocker.MagicMock()
        checkpoint_manager = mocker.MagicMock()
        test_validation_agent = mocker.MagicMock()
        ...
        logger.info.assert_called_with(f"Validating test for function: {mock_generated_test.function.name}")
    ...
    def test_validate_test_validation_error(self, mocker, mock_generated_test):
        ...
        logger.error.assert_called_with(f"Error validating test for {mock_generated_test.function.name}: Validation error")